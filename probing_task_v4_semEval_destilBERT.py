...
class AttentionSentimentClassifier(nn.Module):
    def __init__(self):
        ...
        # self.relu = nn.ReLU()
        self.fc = nn.Linear(1 * 768, 20)
        self.classify = nn.Sigmoid()

    def forward(self, input):
        return self.classify(self.fc(input.flatten(1)))
        #return self.classify(self.fc(self.relu(self.fc2(input.flatten(1)))))


from datasets import load_dataset
train_dataset_hg = load_dataset('sem_eval_2010_task_8', split='train')
test_dataset_hg  = load_dataset('sem_eval_2010_task_8', split='test')

print(train_dataset_hg[0])
#print(train_dataset_hg[0][0])
#tzt

def REL_probing_classifier_training(train_set, model, tokenizer):
    ...
    for epoch in range(0,2):
        ...
        for index, (inputs, labels) in enumerate(train_set):
            ...
            if index % 25 == 24:
                ...
                #break
        for layer in range(13):
            path = './probing_semEval/nn_model_epoch' + str(epoch) + '_layer' + str(layer) + '.pth'
            torch.save(mlp_classifier[layer].state_dict(), path)
        print("model saved")
        print("training finished")


def REL_probing_classifier_testing(test_set, model, tokenizer):
    ...
    for layer in range(13):
        mlp_classifier.append(AttentionSentimentClassifier())
        path = './probing_semEval/nn_model_epoch' + str(load_from_epoch) + '_layer' + str(layer) + '.pth'
        mlp_classifier[layer].load_state_dict(torch.load(path))

    print("evaluation started")
    act_loss = [0.0]*14
    for index, (inputs, labels) in enumerate (test_set):
        if (index > len(test_set) / 2):
            continue
        output = model(torch.tensor([inputs]), output_hidden_states=True, return_dict=True)
...
if __name__ == '__main__':
    ...
    model_name = 'howey/bert-base-uncased-qqp'
    ...
    for data in train_dataset_hg:
        input_ids = tokenizer.encode(data['sentence'])
        train_dataset.append((input_ids, data['relation']))
    test_dataset = []
    for data in test_dataset_hg:
        input_ids = tokenizer.encode(data['sentence'])
        test_dataset.append([input_ids, data['relation']])
    print(len(train_dataset))
    print(len(test_dataset))
    #REL_probing_classifier_training (train_dataset, model, tokenizer)
    REL_probing_classifier_testing  (test_dataset, model, tokenizer)
